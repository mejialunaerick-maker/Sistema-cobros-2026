...
// URL de NGROK para tus pruebas locales
const URL_BASE = "https://evelina-lanuginose-overprosperously.ngrok-free.dev";


// ==========================================
// 🔑 ENDPOINT DE LOGIN RESTAURADO (ADMIN / AUXILIAR)
// ==========================================
app.post('/api/login', async (req, res) => {
    const { correo, contrasena } = req.body; // El HTML envía correo y contrasena
    ...
    try {
        // Consultamos la tabla usando tus columnas exactas: contraseña (con ñ) y rol_id
        const query = `
        ...
        // 2. Validar si el usuario está activo en el sistema
        if (!usuario.activo) {
            return res.status(403).json({ success: false, error: "Esta cuenta se encuentra desactivada." });
        }

        // 3. Validar la contraseña (respetando la ñ de la columna contraseña)
        if (usuario.contraseña !== contrasena) {
            return res.status(401).json({ success: false, error: "La contraseña es incorrecta." });
        }

        // Mapeamos el rol_id para que el HTML sepa a dónde redireccionar
        // rol_id = 1 -> Administrador (redirecciona a admin.html)
        // rol_id = 2 -> Auxiliar/Personal (redirecciona a auxiliar.html)
        const rolTexto = usuario.rol_id === 1 ? 'Administrador' : 'Personal';

        console.log(`🔑 Sesión iniciada correctamente por: ${usuario.nombre} (${rolTexto})`);
        res.json({
            success: true,
        ...
        console.error("❌ Error en el proceso de login global:", err.message);
